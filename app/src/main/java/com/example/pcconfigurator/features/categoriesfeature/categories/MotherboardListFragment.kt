...
import com.example.pcconfigurator.utils.MotherboardAdapter
...
class MotherboardListFragment : Fragment(R.layout.fragment_cooler_list) {
    ...
    private fun setupRecyclerView() {
        binding.coolerRecyclerView.apply {
            adapter = this@MotherboardListFragment.adapter
            layoutManager = LinearLayoutManager(requireContext())
        }
    }

    private fun observeViewModel() {
        viewModel.motherboards
            .onEach { motherboards ->
                adapter.submitList(motherboards)
            }
