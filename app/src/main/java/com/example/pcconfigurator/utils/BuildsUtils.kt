...
class BuildHolder(val binding: BuildItemBinding) : ViewHolder(binding.root) {
    fun onBind(build: Build) = with(binding) {
        buildItemTV.text = build.title
    }
}

class BuildComparator : DiffUtil.ItemCallback<Build>() {
    override fun areItemsTheSame(oldItem: Build, newItem: Build): Boolean = oldItem.id == newItem.id

    override fun areContentsTheSame(oldItem: Build, newItem: Build): Boolean = oldItem == newItem
}
