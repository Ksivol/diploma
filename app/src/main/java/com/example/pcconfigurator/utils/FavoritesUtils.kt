...
class FavoritesAdapter : ListAdapter<Favorites, FavoritesHolder>(FavoritesComparator()) {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): FavoritesHolder =
        FavoritesHolder(FavoritesItemBinding.inflate(LayoutInflater.from(parent.context), parent, false))

    override fun onBindViewHolder(holder: FavoritesHolder, position: Int): Unit =
        holder.onBind(getItem(position))
}

class FavoritesHolder(private val binding: FavoritesItemBinding) : ViewHolder(binding.root) {
    fun onBind(favorites: Favorites) = with(binding) {
...
class FavoritesComparator : DiffUtil.ItemCallback<Favorites>() {
    override fun areItemsTheSame(oldItem: Favorites, newItem: Favorites): Boolean = oldItem.id == newItem.id

    override fun areContentsTheSame(oldItem: Favorites, newItem: Favorites): Boolean = oldItem == newItem
}